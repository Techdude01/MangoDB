...
def account_settings():
    ...
    cursor.execute("SELECT userID FROM UserTag WHERE userName = %s", (username,))
    ...
    if request.method == 'POST':
        ...
        selected_tag_ids = request.form.getlist('UserTag')  # List of selected tag IDs
        ...
        if new_first_name or new_last_name:
            ...
            # Update tags
            cursor.execute("DELETE FROM UserTag WHERE userID = %s", (user_id,))
            for tag_id in selected_tag_ids:
                cursor.execute("INSERT INTO UserTag (userID, tagID) VALUES (%s, %s)", (user_id, tag_id))
